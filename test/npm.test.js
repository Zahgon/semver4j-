...
// Mirrors NpmSemverTest (parameterized): version, rangeExpression, expected
const PARAMS = [
...
describe('NpmSemverTest', () => {
  PARAMS.forEach(([version, rangeExpression, expected], i) => {
    test(`test[${i}]`, () => {
      assert.strictEqual(
        new Semver(version, SemverType.NPM).satisfies(rangeExpression),
        expected,
        `${version} , ${rangeExpression}`
      );
    });
  });
});
